        ...
        if event.type == VkEventType.MESSAGE_NEW:
            ...
            if event.to_me:
                ...
                if len(pattern.findall(request)) > 0:
                    ...
                    q = conn.session.query(Favorite.first_name, Favorite.last_name, Favorite.url_profile)\
                        .select_from(Favorite).filter(Favorite.user_id == f'{user_id}').all()
...
if __name__ == '__main__':
    ...
    run_bot(vk_access_token, DB_PORT, DB_NAME, DB_HOST,
            DB_ENGINE, DB_USERNAME, DB_PASSWORD)
